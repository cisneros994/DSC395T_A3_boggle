...
def test_word_search():
    ...
    # Added Cases
    assert game.find_word_in_board("zsdf") is None # Case where the first letter can't be found
    assert game.find_word_in_board("aln") == [(1,0), (1,1), (2,1)] # Case where the first coordinate found needs to go to the 2nd (or higher) occurrence of the 1st letter
    assert game.find_word_in_board("ale") == [(1,0), (1,1), (0,1)] # multiple paths for the same word #(0,0) or (0,1) or (1,2) for 'e'
    assert game.find_word_in_board("lest") is None
    # Case where 'pope' would pass with the 2nd P found
    game2 = MyGameManager()
    example_board_2 = [
        ["A", "L", "E", "P"],
        ["H", "N", "B", "O"],
    ...
    game2.new_game(len(example_board_2), CUBE_FILE, game_dict)
    game2.set_game(example_board_2)
    assert game2.find_word_in_board("pope") == [(3,0),(3,1),(3,2),(3,3)]
