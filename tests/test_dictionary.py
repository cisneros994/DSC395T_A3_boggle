...
def test_contains_all_example():
    ...
    # Added case if non-existent word
    assert not game_dict.contains('zzz')
    # Added case if a prefix but not a word
    assert not game_dict.contains('po')
...
def test_prefixes_example():
    ...
    # Added case to pass test if it can't find the pfx
    pfx_bad = "zzz"
    assert not game_dict.is_prefix(pfx_bad)
